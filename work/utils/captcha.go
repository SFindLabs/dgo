...
const txtChars = "ABCDEFGHJKMNPQRSTUVWXYabcdefghjkmnpqrstuvwxy3456789"
...
func (captcha *CaptchaImage) SaveImageToBase64String(imageFormat ImageFormat) (string, error) {
	var b bytes.Buffer
	var err error
	base64Str := ""
	if imageFormat == ImageFormatPng {
		err = png.Encode(&b, captcha.nrgba)
		if err == nil {
			base64Str = "data:image/png;base64," + base64.StdEncoding.EncodeToString(b.Bytes())
		}
	} else if imageFormat == ImageFormatJpeg {
		err = jpeg.Encode(&b, captcha.nrgba, &jpeg.Options{Quality: 100})
		if err == nil {
			base64Str = "data:image/jpeg;base64," + base64.StdEncoding.EncodeToString(b.Bytes())
		}
	} else if imageFormat == ImageFormatGif {
		err = gif.Encode(&b, captcha.nrgba, &gif.Options{NumColors: 256})
		if err == nil {
			base64Str = "data:image/gif;base64," + base64.StdEncoding.EncodeToString(b.Bytes())
		}
	} else {
		err = errors.New("not supported image format")
	}
...
func (captcha *CaptchaImage) DrawText(text string) *CaptchaImage {
	...
	for i, s := range text {
		...
		//fontSize := float64(captcha.height) / (1 + float64(r.Intn(8))/float64(9))
		fontSize := float64(RangeRand(24, 30))
		...
		x := int(RangeRand(int64(4+int(fontWidth)*i), int64(int(fontWidth)*(i+1)-13)))

		y := int(RangeRand(int64(captcha.height/2+4), int64(int(captcha.height)-8)))
